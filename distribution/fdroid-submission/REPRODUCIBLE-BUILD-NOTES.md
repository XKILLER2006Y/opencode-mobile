...
## Update (2026-08-04) � expo-notifications FCM stub recipe reworked + build-verified

The old F-Droid patch set copied patched copies of upstream files under
`expo/modules/notifications/` and deleted two upstream files. With
expo-notifications SDK 54+ (module version 57.0.8) that layout no longer
compiles: the module declares no FCM dependency, so upstream code cannot
resolve `com.google.firebase.messaging.*` at all.

New, validated approach (mirrored verbatim in
`.github/workflows/build.yml` `build-fdroid` and
`distribution/fdroid-submission/metadata.yml` `Builds:.prebuild`):

1. Copy shadow stubs at the package root, not the module root:
   `cp -a fdroid/expo-notifications-patches/. node_modules/expo-notifications/android/src/main/java/`
   The stubs (`com/google/firebase/messaging/{FirebaseMessaging,FirebaseMessagingService,RemoteMessage}.kt`)
   fail every FCM operation with an explicit "not available in F-Droid build" error.
2. Provide the real `com.google.android.gms:play-services-tasks:18.0.2` artifact
   (pure Java, no Play services binary requirement; already pulled transitively
   by expo-store-review) so upstream imports of `com.google.android.gms.tasks.Task`
   compile � do NOT stub Task, it collides at dex-merge time.
3. Do NOT delete upstream `RemoteMessageSerializer.java` /
   `FirebaseNotificationTrigger.kt` � they compile fine against the stubs.

Verified: `assembleRelease` BUILD SUCCESSFUL end-to-end in WSL Ubuntu (same JDK
21 toolchain, same sed/cp steps), and replaying the recipe steps against the
pristine tree produces byte-identical build.gradle + stub files. Remaining
check before filing the F-Droid MR is still `fdroid build cc.agentlabs.opencode:39`.
