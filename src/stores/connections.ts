...
interface ConnectionsState {
  ...
  // Ping health of a saved connection
  pingHealth: (connectionId: string) => Promise<boolean>
}
...
export const useConnections = create<ConnectionsState>((set, get) => ({
  ...
  pingHealth: async (connectionId: string) => {
    const conn = get().connections.find((c) => c.id === connectionId)
    if (!conn) return false
    try {
      const password = await SecureStore.getItemAsync(`${PASSWORDS_PREFIX}${connectionId}`)
      const res = await get().testConnection(conn, "edit_test", password ?? undefined)
      return res.ok
    } catch {
      return false
    }
