        ...
        for await (const event of client.global.events(currentController.signal)) {
          ...
          switch (type) {
            ...
            case "message.part.updated": {
              ...
              // Update status text from the latest part (skip state mutation if label hasn't changed)
              const sessionID = part.sessionID
              if (sessionID) {
                const label = statusFromPart(part)
                set((state) => {
                  if (state.statusText[sessionID] === label) return state
                  return {
                    statusText: { ...state.statusText, [sessionID]: label },
                  }
                })
              }
